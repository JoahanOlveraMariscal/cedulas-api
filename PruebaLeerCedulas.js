...
// ===== ENV =====
const { PROXY_URL, PROXY_USER, PROXY_PASS } = process.env;
...
async function tcpProbe(host, port = 443, timeoutMs = 10000) {
  return new Promise(resolve => {
    ...
    const finish = (ok, extra={}) => { if (done) return; done = true; try{sock.destroy();}catch{}; resolve({ ok, ms: Date.now()-t0, ...extra }); };
...
async function tlsProbe(host, port = 443, timeoutMs = 12000) {
  return new Promise(resolve => {
    ...
    const s = tls.connect({ host, port, servername: host, rejectUnauthorized: true, timeout: timeoutMs }, () => {
      ...
      resolve({ ok: true, ms: Date.now()-t0, alpn: s.alpnProtocol || null, authorized: s.authorized, cipher: s.getCipher?.() || null, peerCN: peer.subject?.CN || null });
      s.destroy();
    });
    s.on('error', e => { resolve({ ok: false, error: String(e?.code || e?.message || e) }); s.destroy(); });
    s.setTimeout(timeoutMs, () => { resolve({ ok: false, error: 'TLS_TIMEOUT' }); s.destroy(); });
  });
}
    ...
    for(const f of page.frames()){
      const inputs = await f.evaluate(()=> {
        ...
        return Array.from(document.querySelectorAll('input, select, textarea'))
          .filter(vis).map(el=>({ tag:el.tagName.toLowerCase(), id:el.id||'', fcn:el.getAttribute('formcontrolname')||'', ph:el.getAttribute('placeholder')||'' }));
      });
...
// ===== Arranque =====
const server = app.listen(PORT, '0.0.0.0', () => console.log('Playwright listo en puerto', PORT));
